...
#
# Usage: ./gmx_extract.sh -s <tpr> -f <xtc> [OPTIONS]
#
# Required:
...
#                             (default: "<outdir>/gmx_extract_commands.sh")
...
#   ./gmx_extract.sh -s md.tpr -f md.xtc
#
#   # Only extract a 0-100 ns slice of the Protein group, no PBC/center/fit/pdb
#   ./gmx_extract.sh -s md.tpr -f md.xtc --steps extract -b 0 -e 100000
#
#   # Extract + remove PBC only (no centering/fit/pdb)
#   ./gmx_extract.sh -s md.tpr -f md.xtc --steps pbc -b 0 -e 100000
#
#   # Resample the whole system every 100 ps, no PBC treatment
#   ./gmx_extract.sh -s md.tpr -f md.xtc -g System --steps extract --dt 100
#
#   # Dry run of the full pipeline, saving the commands to a script
#   ./gmx_extract.sh -s md.tpr -f md.xtc --dry-run --save-script run_viz.sh
#
...
save_script="${save_script:-${outdir}/gmx_extract_commands.sh}"
...
if [[ -n "${save_script}" ]]; then
    {
        echo "#!/bin/bash -e"
        echo "# Commands executed by gmx_extract.sh"
