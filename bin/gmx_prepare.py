...
Usage: python gmx_prepare.py <pdb_file> [options]
"""
...
def set_command_script_path(path):
    ...
    with open(path, 'w') as f:
        ...
        f.write("# Generated by gmx_prepare.py\n")
...
def automate_gromacs(args):
    ...
    # Initialize command logging script (written even in --dry-run mode)
...
Examples:
  ...
  gmx_prepare.py protein.pdb
  
  # Custom force field and water model
  gmx_prepare.py protein.pdb --forcefield amber99sb-ildn --water-model tip3p
  
  # Larger box with dodecahedral shape
  gmx_prepare.py protein.pdb --box-distance 1.5 --box-type dodecahedron
  
  # Skip pdb2gmx - use existing topology and structure
  gmx_prepare.py --topology protein.top --structure protein.gro
  
  # Preview commands without executing (dry-run)
  gmx_prepare.py protein.pdb --dry-run
  
  # Verbose output for debugging
  gmx_prepare.py protein.pdb --verbose
        """)
    ...
    parser.add_argument("--dry-run", action="store_true", 
                       help="Log commands without executing them (preview mode)")
    parser.add_argument("-v", "--verbose", action="store_true",
                       help="Verbose output (show detailed progress)")
    parser.add_argument("--commands-file", default="gmx_prepare_commands.sh",
                       help="Output file for command logging (default: gmx_prepare_commands.sh)")
